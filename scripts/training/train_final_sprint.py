...
ind_raw AS ( SELECT stkcd, date, Nindnme as industry FROM TRD_Co WHERE date <= '{end_date}' ),
shrs_raw AS ( SELECT stkcd, date, a_circulated_share FROM stk_shares_raw WHERE date <= '{end_date}' ),
bs AS (
...
def _neutralize_strict(df, features):
    print(f"[NEUTRAL] Performing strict Industry+Size regression on {len(df)} rows...")
    df = df.copy()
    ...
    for d in dates:
        ...
        ind_dummies = pd.get_dummies(day_df['industry'], drop_first=True)
        X = np.column_stack([np.ones(len(day_df)), day_df['log_cap'].values, ind_dummies.values.astype(float)])
        for f in features:
            y = day_df[f].fillna(day_df[f].median()).values
            beta, _, _, _ = np.linalg.lstsq(X, y, rcond=None)
            day_df[f] = y - X.dot(beta)
...
def _load_split_df(conn: duckdb.DuckDBPyConnection, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    df = _neutralize_strict(df, FEATURE_COLS_64)
...
def main():
    ...
    for i in range(args.num_seeds):
        ...
        params = {"objective": "regression", "metric": "l2", "boosting_type": "gbdt", "learning_rate": 0.01, "num_leaves": 63, "max_depth": 10, "feature_fraction": 0.6, "bagging_fraction": 0.7, "seed": seed, "verbosity": -1}
        m = lgb.train(params, dtrain, num_boost_round=1000, valid_sets=[dvalid], callbacks=[lgb.early_stopping(100)])
    ...
    print("\n--- FINAL SPRINT RESULT ---")
    print(json.dumps({"rank_ic": daily_ic.mean(), "iric": iric, "positive_rate": (daily_ic > 0).mean()}, indent=2))
    
    tag = datetime.now().strftime("%Y%m%d_%H%M%S")
    out = Path(args.out_dir) / f"final_sprint_{args.num_seeds}s_{tag}"
