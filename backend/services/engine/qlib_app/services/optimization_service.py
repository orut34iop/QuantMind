...
class OptimizationService:
    ...
    async def run_optimization(
        ...
        task_log.info("start", "开始参数优化任务", total_combinations=total_combinations)
        ...
        async def run_single_task(combo):
            async with semaphore:
                if await is_cancelled():
                    return None, {"params": dict(zip(param_names, combo)), "cancelled": True}
                ...
                try:
                    ...
                    return OptimizationTaskResult(params=params, metrics=backtest_res), None
                except Exception as e:
                    task_log.error("task_failed", "Optimization task failed", params=params, error=e)
        ...
        for future in asyncio.as_completed(tasks):
            ...
            completed_count += 1
            current_params = result.params if result is not None else (error_info.get("params") if error_info else None)
            if result is not None:
                all_results.append(result)
                if best_result is None:
                    best_result = result
                else:
                    current_value = getattr(result.metrics, request.optimization_target, None)
                    best_value = getattr(best_result.metrics, request.optimization_target, None)
                    current_value = (
                        current_value if current_value is not None else (-1e9 if direction == "maximize" else 1e9)
                    )
                    best_value = best_value if best_value is not None else (-1e9 if direction == "maximize" else 1e9)
                    is_better = current_value > best_value if direction == "maximize" else current_value < best_value
                    if is_better:
                        best_result = result
            else:
                failed_count += 1
            ...
            if progress_callback:
                progress_payload = {
                    "optimization_id": optimization_id,
                    "progress": completed_count / total_combinations if total_combinations else 1.0,
                    "status": "running",
                    ...
                    "best_metric_value": (
                        getattr(best_result.metrics, request.optimization_target, None) if best_result else None
                    ),
                    "result_summary": {
        ...
        if all_results:
            ...
            sorted_results = sorted(
                all_results,
                key=lambda x: (getattr(x.metrics, target) if getattr(x.metrics, target) is not None else -1e9),
                reverse=reverse,
            )
