...
> **当前状态（2026-03-18）**：QuantBot 已通过 `quantmind-api` 适配 CoPaw，进入测试接入阶段。
> 已启用：聊天、会话管理、历史消息、健康检查，以及基于 CoPaw 的新闻查阅、邮件管理、Word/PDF/Excel/PPT 处理、定时任务、钉钉频道接入等能力入口。
...
前端 → `quantmind-api (:8000)` → CoPaw `(:8088)`

- 前端**不直连** CoPaw，统一调用 `quantmind-api` 暴露的 `/api/v1/openclaw/*`。
- 网关负责会话 ID 映射、字段补默认值、hop-by-hop header 清洗。
- 消息链路当前采用**网关直传 CoPaw SSE**，前端按增量事件实时渲染回复。
- 附件链路当前采用**共享卷写入**：前端上传到 `quantmind-api`，网关把文件写入 `copaw-data` 共享卷，并把 CoPaw 容器可见路径注入到消息上下文。
- 测试阶段无上游访问密钥；如 CoPaw 后续增加鉴权，统一在网关适配层处理，不由前端承担。
...
| 区域 | 组件 | 当前用途 |
|------|------|----------|
| 左侧栏 | `GuideSection` + `QuickActions` | 展示 CoPaw 已接入技能与快捷示例 |
| 中间区 | `ChatContainer` | 对话区（发送消息、展示回复） |
| 右侧栏 | `TaskPanelContainer`（降级） | 会话管理面板（列表、切换、重命名、删除） |

顶部状态栏每 30 秒通过 `GET /api/v1/openclaw/health` 探活；前端当前仅以该请求是否成功返回来判断 `quantmind-api` 网关是否可达，不再展示 CoPaw 上游状态。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/api/v1/openclaw/chat` | 发送消息（映射到 CoPaw `/api/agent/process`） |
...
| `GET`  | `/api/v1/openclaw/health` | 健康检查（合成探活 CoPaw） |
...
- 降级模式：当 `quantmind-api` 当前容器缺少 CoPaw/鉴权配置导致 `/api/v1/openclaw/sessions` 返回 `503` 时，前端会自动退化为本地临时会话，不再把会话列表与建会话失败当成致命错误。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `COPAW_BASE_URL` | `http://copaw:8088` | CoPaw 上游地址（后端读取） |
| `COPAW_CHANNEL` | `console` | CoPaw channel |
| `COPAW_TIMEOUT_SECONDS` | `60` | 上游请求超时（秒） |
| `COPAW_SHARED_FILES_DIR` | `/copaw-shared` | `quantmind-api` 容器内的共享卷挂载点 |
| `COPAW_SHARED_VISIBLE_DIR` | `/app/working` | CoPaw 容器内读取附件时看到的根目录 |
...
- **后端适配层**：FastAPI + httpx（`copaw_proxy.py`），`quantmind-api` 统一暴露

## 后续规划

- 恢复独立任务系统（待 CoPaw 任务接口稳定后接入）
- 完善附件历史回显与结果结构化展示
- 接入模型、技能、工具等高级管理能力

---

**Version**: 2.0.0（CoPaw 测试接入）
